...
# OpenRouter fallback модели (проверенные рабочие)
    ...
    try:
        ...
        if start != -1 and end != -1:
            ...
            if isinstance(items, list):
                valid_types = {"main_pain", "loved_one", "promise", "unfinished_question",
                               "life_event", "fear", "goal", "breakthrough"}
                return [
                    it for it in items[:3]
                    if isinstance(it, dict) and it.get("memory_type") in valid_types and it.get("content")
                ]
        return []
    except Exception as e:
