...
async def _gemini_single_request(
    ...
    payload = {
        "contents": contents,
        "generationConfig": {"temperature": 0.85, "maxOutputTokens": max_tokens, "topP": 0.92},
    }
    try:
...
async def _gemini_call(contents: list[dict], timeout: float = 8.0, max_tokens: int = 700) -> str:
    ...
    text, status = await _gemini_single_request(model, api_key, contents, timeout, max_tokens)
    if status == 200:
        return text
    ...
    if status in (404, 429):
        for fallback in GEMINI_MODELS:
            ...
            text, status = await _gemini_single_request(fallback, api_key, contents, timeout, max_tokens)
            if status == 200:
                return text
            if status == 429:
                break
...
async def _openai_compatible_call(
    ...
    max_tokens по умолчанию 700 — для диалоговых ответов это достаточно,
    генерация быстрее, чем с 1024. Для раскладов вызывающий код явно
    передаёт большее значение.
    """
    payload = {
        "model": model,
        "messages": messages,
...
async def _groq_call(contents: list[dict], timeout: float = 8.0, max_tokens: int = 700) -> str:
    api_key = _get_api_key()
    if not api_key:
        return "У меня нет доступа к внутреннему голосу... Попробуй позже."
    ...
    text, status = await _openai_compatible_call(GROQ_BASE_URL, api_key, model, messages, timeout, max_tokens)
    if status == 200:
        return text
    if status == 429:
        return "Силы пока на исходе, милый человек. Подожди немного — и я снова буду готова говорить."
...
async def _openrouter_call(contents: list[dict], timeout: float = 8.0, max_tokens: int = 700) -> str:
    api_key = _get_api_key()
    if not api_key:
        return "У меня нет доступа к внутреннему голосу... Попробуй позже."
    ...
    text, status = await _openai_compatible_call(OPENROUTER_BASE_URL, api_key, model, messages, timeout, max_tokens)
    if status == 200:
        return text
    ...
    if status in (429, 404, 400, 503, -2):
        ...
        for fb in OPENROUTER_FALLBACKS:
            ...
            text, fb_status = await _openai_compatible_call(OPENROUTER_BASE_URL, api_key, fb, messages, timeout, max_tokens)
            if fb_status == 200:
                return text
...
async def _llm_call(contents: list[dict], timeout: float = 8.0, max_tokens: int = 700) -> str:
    """Единый вызов LLM через выбранного провайдера."""
    provider = os.getenv("LLM_PROVIDER", "gemini").lower()

    if provider == "groq":
        return await _groq_call(contents, timeout, max_tokens)
    elif provider == "openrouter":
        return await _openrouter_call(contents, timeout, max_tokens)
    else:  # gemini (default)
        return await _gemini_call(contents, timeout, max_tokens)
...
async def generate_response(
    ...
    return await _llm_call(contents, timeout=6.0, max_tokens=500)
...
Только вопрос, 1-3 предложения. От лица Софии, тепло, с лёгкой интригой. Без эмодзи в этом сообщении."""
    ...
    return await _llm_call(contents, timeout=6.0, max_tokens=200)
...
Говори как мудрая старушка-хранительница. Не используй астрологические термины напрямую (не «Лев», «Марс», «дом»). Образно, тепло, с интригой. В каждой части — про этого конкретного человека, опирайся на дату рождения и то, что он уже сказал."""
    ...
    return await _llm_call(contents, timeout=10.0, max_tokens=1200)
...
Говори как София: образно, тепло, без сухих значений из учебника. Не более 6-8 предложений всего."""
    ...
    return await _llm_call(contents, timeout=6.0, max_tokens=400)
...
Только текст от лица Софии. Без эмодзи в начале — это интимный момент встречи."""
    ...
    return await _llm_call(contents, timeout=6.0, max_tokens=300)
...
ВАЖНО по нравственному кодексу: никаких точных дат, никаких категоричных предсказаний болезней/смерти. «Вероятный путь», «обрати внимание», «период может быть сложным»."""
    ...
    return await _llm_call(contents, timeout=10.0, max_tokens=1500)
...
В конце: «🃏 Карта дня: {card_name}»"""
    ...
    return await _llm_call(contents, timeout=7.0, max_tokens=500)
...
Подпись в конце: «🌙 Твоя София»"""
    ...
    return await _llm_call(contents, timeout=7.0, max_tokens=600)
...
{dialog}"""
    ...
    try:
        ...
        result = await _llm_call(contents, timeout=5.0, max_tokens=400)
...
Только текст сводки. Без лишних комментариев."""
    ...
    try:
        contents = [{"role": "user", "parts": [{"text": prompt}]}]
        result = await _llm_call(contents, timeout=5.0, max_tokens=300)
