...
Round 4: зодиак в промптах, реальные имена карт Таро, улучшенные расклады,
сводка диалога для длинных разговоров.
...
# ──────────────────────── Системный промпт Софии (концепция v2 + round 4) ────────────────────────
...
Перед ответом тебе передаётся сводка о пользователе: имя, дата рождения, знак зодиака, важные факты, последние сообщения. Используй факты естественно: «Помнишь, ты рассказывал про сына...» или «Тогда тебя тревожила работа. Стало легче?» Не перечисляй факты списком. Вплетай их в разговор.
...
def _build_context(name: str, birth_date: str, facts: list[dict], history: list[dict],
                   emotional: list[dict] = None, gender: str = "", age_group: str = "",
                   zodiac: str = "", conversation_summary: str = "") -> str:
    """Собираем контекст для LLM с эмоциональной памятью и зодиаком."""
    lines = [f"Имя пользователя: {name}"]
    if birth_date:
        lines.append(f"Дата рождения: {birth_date}")
    if zodiac:
        lines.append(f"Знак зодиака: {zodiac}")
    if gender:
    ...
    if conversation_summary:
        lines.append(f"Сводка более раннего диалога: {conversation_summary}")
...
async def generate_response(
    ...
    zodiac: str = "", conversation_summary: str = ""
) -> str:
    """Основной ответ Софии в диалоге."""
    from datetime import datetime as _dt
    context = _build_context(user_name, birth_date, facts, history, emotional, gender, age_group, zodiac, conversation_summary)
...
async def generate_probing_question(name: str, birth_date: str, zodiac: str = "") -> str:
    """Один вопрос прощупывания — чтобы человек почувствовал «она меня понимает»."""
    zodiac_line = f"\nЗнак зодиака: {zodiac}" if zodiac else ""
    prompt = f"""Ты только что узнала, что пользователя зовут {name}, дата рождения {birth_date}.{zodiac_line}
...
async def generate_fate_card(
    ...
    birth_place: Optional[str] = None, probing_answer: str = "", zodiac: str = ""
) -> str:
...
Дата рождения: {birth_date}{zodiac_line}
...
async def generate_single_card(name: str, question_context: str = "", card_name: str = "") -> str:
    ...
    card_hint = f"\nКарта, которую вытянул человек: {card_name}." if card_name else ""

    prompt = f"""Вытяни одну карту Таро для {name}.{context_line}{card_hint}

Случайно выбери число от 1 до 78. Это номер карты в классической колоде Таро (Райдер-Уэйт):
- 1–22 — Старшие Арканы (Шут=1, Маг=2, ... Мир=22)
- 23–36 — Жезлы (Туз-Король)
- 37–50 — Кубки (Туз-Король)
- 51–64 — Мечи (Туз-Король)
- 65–78 — Пентаклей (Туз-Король)
...
async def generate_return_greeting(
    ...
    if zodiac:
        context_parts.append(f"Знак зодиака: {zodiac}")
...
async def generate_daily_horoscope(
    name: str, birth_date: str, emotional: list[dict] = None, zodiac: str = ""
) -> str:
    ...
    prompt = f"""Сегодня {today_str}. Пользователя зовут {name}, дата рождения {birth_date}.{zodiac_line}{em_lines}
...
async def generate_taro_reading(name: str, question: str, numbers: list[int],
                                full: bool = False, card_names: dict[int, str] = None) -> str:
    """Расклад Таро — малый (5 карт) или полный (20 карт).
    card_names: маппинг number → имя карты (из fsm.get_tarot_card_name)."""
    ...
    # Подставляем реальные имена карт
    cards_info = []
    for i, n in enumerate(numbers[:count]):
        cname = card_names.get(n, f"Карта #{n}") if card_names else f"Карта #{n}"
        cards_info.append(f"  Позиция {i+1}: число {n} → {cname}")
    cards_text = "\n".join(cards_info)

    spread_kind = "полный" if full else "малый"
    prompt = f"""Сделай {spread_kind} расклад Таро для {name}.
...
Для каждой позиции: используй реальное имя карты, дай образное толкование.
...
async def generate_horoscope(
    name: str, birth_date: str, birth_time: Optional[str] = None,
    birth_place: Optional[str] = None, concerns: str = "", zodiac: str = ""
) -> str:
...
# ──────────────────────── Извлечение эмоциональной памяти ────────────────────────
...
# ──────────────────────── Сводка диалога (для сжатия контекста) ────────────────────────

async def generate_conversation_summary(messages: list[dict]) -> str:
    """Генерирует краткую сводку более ранних сообщений для сжатия контекста.
    Используется, когда история диалога длинная — старые сообщения заменяются сводкой."""
    if not messages:
        return ""

    dialog = "\n".join([
        f"{'София' if m['role'] == 'sofia' else 'Человек'}: {m['content'][:200]}"
        for m in messages
    ])

    prompt = f"""Составь краткую сводку этого диалога (2-4 предложения).
Опиши: о чём говорил человек, что его тревожило, какие темы обсуждались, были ли важные выводы или обещания.

Диалог:
{dialog}

Только текст сводки. Без лишних комментариев."""
...
async def generate_mood_checkin(name: str, emotional: list[dict], last_topic: str = "",
                                zodiac: str = "") -> str:
